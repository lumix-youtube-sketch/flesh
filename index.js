...
const cleanBodyText = (raw, title = '') => {
    ...
    const banned = /(подписыв|подпиш|реклама|erid|промокод|ggsel|vk\.com|t\.me|telegram|дзен|youtube|rutube|@)/i;
...
const CATEGORIES = {
    ...
    sport: {
        name: '⚽ Спорт',
        urls: [
            { u: 'https://www.championat.com/rss/news.xml', n: 'Чемпионат' },
            { u: 'https://sport24.ru/rss/news', n: 'Sport24' },
            { u: 'https://rsport.ria.ru/export/rss2/archive/index.xml', n: 'РИА Спорт' }
        ]
    },
    music: {
        name: '🎵 Музыка',
        urls: [
            ...
            { u: 'https://www.muz-tv.ru/news/rss/', n: 'МУЗ-ТВ' }
        ]
    }
};
                ...
                try {
                    ...
                    for (const item of (feed.items || []).slice(0, 14)) {
                        ...
                        if (Repo.saveNews({
                            ...
                            image: image || (forcedByTestWord ? 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/640px-No-Image-Placeholder.svg.png' : null),
                            video,
                            source: src.n,
                            cat: src.cat,
                            reg: src.reg,
